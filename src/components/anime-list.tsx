...
const AnimeList = ({
  ...
  return (
    <Link
      href={`/${id}`}
      className="flex items-start space-x-3 p-4 rounded-md shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer"
    >
      <Image
        src={image}
        alt={`${title}_${image}`}
        width="60"
        height="88"
        className="flex-none rounded-md bg-slate-100 w-auto h-auto"
      />
